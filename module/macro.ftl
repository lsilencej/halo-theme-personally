<#macro head title>
    ...
    <head>
        ...
        <script>
            ...
            hm.src = "https://hm.baidu.com/hm.js?8bbef3b8f80c21c3730669ae29dfcc09";
        ...
        <div class="copyright text-767676 dark-text-[#999]">
            ...
            <#if settings.icp_beian?? || settings.ga_beian_content??>
                <div class="beian text-xs text-767676 space-y-1 absolute bottom-0 select-none">
                    <#if settings.moe_beian??>
                        <p>
                            <a href="https://icp.gov.moe/?keyword=${settings.moe_beian_code}" target="_blank">${settings.moe_beian!}</a>
                        </p>
                    </#if>
...
<#macro footer>
    ...
    <script>
        var moonMenuListener = function() {
            var contentHeight = $(".moon-main").height() - window.innerHeight;
            if (contentHeight <= 0) return;
            var readHeight = window.pageYOffset - $(".moon-main").offset().top;
        ...
        function back2Bottom() {
            window.scroll({ top: $(".moon-main").height(), behavior: 'smooth' });
        }
    ...
    <script data-no-instant>
        InstantClick.on('change', function() {
            var siteTrend;
            $.ajax({
                url: "https://statistic.lsilencej.top/statistic/siteTrend",
                type: "GET",
                dataType: "json",
                async: false,
            ...
            var siteTrendCharts = echarts.init(document.getElementById("site-trend"));
            siteTrend && siteTrendCharts.setOption(siteTrend);
        });
        InstantClick.init();
        var siteTrend;
        $.ajax({
    ...
    <script data-no-instant>
        InstantClick.on('change', function() {
            var siteArea;
            $.ajax({
                url: "https://statistic.lsilencej.top/statistic/siteArea",
                type: "GET",
                dataType: "json",
                async: false,
                success: function (response) {
                    siteArea = response.data;
                    delete siteArea.series[0].label;
                    delete siteArea.series[0].labelLine;
                }
            });
            var siteAreaCharts = echarts.init(document.getElementById("site-area"));
            siteArea && siteAreaCharts.setOption(siteArea);
        });
        InstantClick.init();
        var siteArea;
        $.ajax({
    ...
    <script data-no-instant>
        InstantClick.on('change', function() {
            var siteSource;
            $.ajax({
                url: "https://statistic.lsilencej.top/statistic/siteSource",
                type: "GET",
                dataType: "json",
                async: false,
                success: function (response) {
            ...
            var siteSourceCharts = echarts.init(document.getElementById("site-source"));
            siteSource && siteSourceCharts.setOption(siteSource);
        });
        InstantClick.init();
        var siteSource;
        $.ajax({
    ...
    <script>
        var calendarChart = echarts.init(document.getElementById('post-calendar'));
        var categoryChart = echarts.init(document.getElementById("category-echarts"));
        var siteTrend = echarts.init(document.getElementById("site-trend"));
        var siteArea = echarts.init(document.getElementById("site-area"));
        var siteSource = echarts.init(document.getElementById("site-source"));
